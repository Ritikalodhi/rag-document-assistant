...
export function MessageBubble({
  ...
  if (message.role === 'user') {
    return (
      <div className="flex justify-end">
        <div className="max-w-[75%] px-4 py-3 rounded-[12px] bg-[rgb(var(--color-surface))] border border-[rgb(var(--color-border))]">
          <p className="text-body text-[rgb(var(--color-text))] leading-relaxed">{message.content}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Assistant header: icon + confidence */}
      <div className="flex items-center gap-3">
        <div className="w-7 h-7 rounded-[6px] bg-[rgb(var(--color-accent))] flex items-center justify-center flex-shrink-0">
          <svg className="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
          </svg>
        </div>
        <span className="text-caption font-medium text-[rgb(var(--color-text-secondary))]">Aperture</span>
      ...
      {/* Assistant message — no chat bubble, rendered as research content */}
      <div className="max-w-full">
        {message.isStreaming && !message.content ? (
          <div className="flex items-center gap-2.5 text-[rgb(var(--color-text-secondary))] py-2">
            <span className="w-1.5 h-1.5 rounded-full bg-[rgb(var(--color-accent))] animate-pulse" />
            <span className="text-body">Searching your documents...</span>
          </div>
        ) : (
          <>
            <MarkdownRenderer content={message.content} />
            {message.isStreaming && (
              <span className="inline-block w-2 h-4 ml-0.5 bg-[rgb(var(--color-accent))] animate-pulse" />
            )}
          </>
        )}
      </div>
      ...
      {message.error && !message.isStreaming && (
        <div className="flex flex-col gap-3 p-4 rounded-[10px] bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
          <p className="text-caption text-red-700 dark:text-red-400">{message.error}</p>
        ...
        <div className="flex flex-wrap gap-1.5" role="list" aria-label="Sources">
          {message.context.map((source, i) => (
            <button
              ...
              className="inline-flex items-center justify-center w-7 h-7 rounded-[6px] text-caption font-medium border border-[rgb(var(--color-border))] text-[rgb(var(--color-text-secondary))] hover:border-[rgb(var(--color-accent))] hover:text-[rgb(var(--color-accent))] hover:bg-[rgb(var(--color-accent-muted))] transition-colors"
              title={source.source}
              role="listitem"
      ...
      {/* "How I found this" collapsible section */}
      {message.context && message.context.length > 0 && !message.isStreaming && (
        <div className="border-t border-[rgb(var(--color-border))] pt-3">
          <button
            onClick={() => setSourcesExpanded(!sourcesExpanded)}
            className="flex items-center gap-2 text-caption font-medium text-[rgb(var(--color-text-secondary))] hover:text-[rgb(var(--color-text))] transition-colors"
      ...
      {!message.isStreaming && !message.error && message.content && (
        <div className="flex items-center gap-1">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-[6px] text-caption text-[rgb(var(--color-text-secondary))] hover:bg-[rgb(var(--color-surface))] hover:text-[rgb(var(--color-text))] transition-colors"
...
function GenericObjectView({ data, depth }: { data: Record<string, unknown>; depth: number }) {
  if (depth > 4) return <span className="text-caption text-[rgb(var(--color-text-secondary))]">[depth limit]</span>;

  return (
    <div className="flex flex-col gap-1 font-code text-caption">
...
function renderValue(value: unknown, depth: number): ReactNode {
  if (value === null || value === undefined) {
    return <span className="text-[rgb(var(--color-text-secondary))]">null</span>;
  }

  if (Array.isArray(value)) {
    if (value.length === 0) return <span className="text-[rgb(var(--color-text-secondary))]">[]</span>;
    return (
      <div className="flex flex-col gap-1 pl-3 border-l border-[rgb(var(--color-border))]">
        {value.map((item, i) => (
          <div key={i} className="flex items-start gap-1">
            <span className="text-[rgb(var(--color-text-secondary))]">{i}:</span>
